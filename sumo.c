  ...
  switch(state)
  {
    ...
    case wait_side_step:     if(walk_left()) {
     ...
     case scanning_enemy: adc6_read = exp_adc_get_avg_channel(ADC_PORT_6); adc5_read = exp_adc_get_avg_channel(ADC_PORT_5);
                          ...
                          if (adc5_read >= 105 + error) turn_to_right = 0;
                          else if (adc5_read < 105 - error && adc6_read >= 300 + error) turn_to_right = 1;

                          cm510_printf("Turn to left: %d\n",turn_to_right);
			                       _delay_ms(100);

                          if (adc6_read >= 400 + error || adc5_read >= 250 + error) {
                            state = attack;
                          }
                          else if (adc6_read <= 300 - error && adc5_read <= 105 - error) {
      ...
      case searching_enemy:   adc6_read = exp_adc_get_avg_channel(ADC_PORT_6); adc5_read = exp_adc_get_avg_channel(ADC_PORT_5);
                              ...
                              if (adc6_read > 300 - error || adc5_read > 105 - error) {
                                mtn_lib_stop_mtn();
                                state=wait_search;
                              }
     ...
     case walking_to_enemy: adc6_read = exp_adc_get_avg_channel(ADC_PORT_6); adc5_read = exp_adc_get_avg_channel(ADC_PORT_5);
                            if (adc6_read >= 400 - error || adc6_read <= 300 + error || adc5_read >= 250 - error || adc5_read <= 105 + error) {
                               mtn_lib_stop_mtn();
                               state=wait_walk;
                             }
